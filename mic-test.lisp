...
;;     (loop  while (null *end*)
;;            do (%al:get-source-i (mem-aref (al-source al) :uint 0) :buffers-processed (al-availbuffers al))
;;               (when (> (mem-ref (al-availbuffers al) :int) 0)
;;                 (%al:source-unqueue-buffers (mem-aref (al-source al) :uint 0) (mem-ref (al-availbuffers al) :int) (al-bufferholder al))
;;                 (loop for ii from 0 below (mem-ref (al-availbuffers al) :int)
;;                       do (setf (al-bufq al) (append (al-bufq al) (list (mem-aref (al-bufferholder al) :uint ii))))))
...
;;                   (kuchipaku imgs moge ol)))
;;               (when (al-bufq al)
;;                 (setf (mem-ref (al-mybuff al) :uint) (car (al-bufq al))
;;                       (al-buf al) (cdr (al-bufq al)))
;;                 (al:buffer-data (mem-ref (al-mybuff al) :uint) #x1101 (al-buffer al) (* *cap_size* (cffi:foreign-type-size :short)) *freq*)
;;                 ;;(when (= count 0)
;;                 ;;  (format t "~s~%" (loop for i from 0 below *cap_size*
;;                 ;;                         collect (mem-aref buffer :short i))))
;;                 (%al:source-queue-buffers (mem-aref (al-source al) :uint 0) 1 (al-mybuff al))
;;                 (%al:get-source-i (mem-aref (al-source al) :uint 0) :source-state (al-state al))
;;                 (when (/= (mem-ref (al-state al) :int) #x1012)
;;                   (al:source-play (mem-aref (al-source al) :uint 0)))
;;                 ;;(incf count)
;;                 )
;;               (gtk-main-iteration-do nil)
;;            )
